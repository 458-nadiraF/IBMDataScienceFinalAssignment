...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class',names="Launch Site", title='Total Success Rate')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_data=filtered_df[filtered_df['Launch Site']==entered_site]
        filtered=filtered_data.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig = px.pie(filtered, values='class count',names="class",title='Total Success Rate')
        return fig
# TASK 4:
